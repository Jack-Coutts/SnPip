...
'''
def fst_graph(positions, array, dividend=1000 ): 
    ...
    for i, num in enumerate(sorted(positions)):
        ...
        n = math.ceil(num/float(dividend))
    ...
    start=index_positions['0']
    stop=index_positions[len(index_positions)]
    step=dividend

    plot=FSTscatter(input, start, stop, step)

    graph=pio.to_html(plot)
